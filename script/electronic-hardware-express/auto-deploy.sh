  ...
  template:
    ...
    spec:
      ...
      - name: ${ProjectName}-service
        ...
        command: ["/bin/bash"]
        args: ["-c","java -jar sxdz_express_cabinet_service-0.0.1-SNAPSHOT.jar &> /data/logs/electronic-hardware-express/sxdz-express-service.log"]
        volumeMounts:
        - name: mysql-cred
          mountPath: "/projected-volume/conf/"
        - name: logs
          mountPath: /data/logs/electronic-hardware-express/
        ports:
        ...
        resources:
          ...
          requests:
            cpu: 2000m
            memory: 2048Mi
#      - name: ${ProjectName}-management
#        image: ${harbor}/project/${ProjectName}:${TAG}
#        command: ["/bin/bash"]
#        args: ["-c","java -jar sxdz_express_cabinet_management-0.0.1-SNAPSHOT.jar &> /data/logs/electronic-hardware-express/sxdz-express-management.log"]
#        volumeMounts:
#        - name: mysql-cred
#          mountPath: "/projected-volume/conf/"
#        - name: logs
